...
resource "google_compute_health_check" "cp_health" {
  count   = var.enable_cp ? 1 : 0
  name    = "${var.environment}-cp-health-health"
  project = var.project_id
...
resource "google_compute_health_check" "pp_health" {
  count   = var.enable_pp ? 1 : 0
  name    = "${var.environment}-pp-health-health"
  project = var.project_id
...
resource "google_compute_health_check" "plant_health" {
  count   = var.enable_plant ? 1 : 0
  name    = "${var.environment}-plant-health-health"
  project = var.project_id
...
resource "google_compute_backend_service" "cp_health" {
  count       = var.enable_cp ? 1 : 0
  name        = "${var.environment}-cp-health-backend"
  project     = var.project_id
  protocol    = "HTTPS"
  port_name   = "http"
  timeout_sec = 30
  ...
  load_balancing_scheme = "EXTERNAL_MANAGED"
  health_checks         = [google_compute_health_check.cp_health[0].id]

  dynamic "backend" {
    for_each = contains(keys(var.backend_negs), "cp_health") ? [1] : []
    content {
      group           = "projects/${var.project_id}/regions/${var.backend_negs["cp_health"].region}/networkEndpointGroups/${var.backend_negs["cp_health"].name}"
      balancing_mode  = "UTILIZATION"
      capacity_scaler = 1.0
    }
  }
...
resource "google_compute_backend_service" "pp_health" {
  count       = var.enable_pp ? 1 : 0
  name        = "${var.environment}-pp-health-backend"
  project     = var.project_id
  protocol    = "HTTPS"
  port_name   = "http"
  timeout_sec = 30
  ...
  load_balancing_scheme = "EXTERNAL_MANAGED"
  health_checks         = [google_compute_health_check.pp_health[0].id]

  dynamic "backend" {
    for_each = contains(keys(var.backend_negs), "pp_health") ? [1] : []
    content {
      group           = "projects/${var.project_id}/regions/${var.backend_negs["pp_health"].region}/networkEndpointGroups/${var.backend_negs["pp_health"].name}"
      balancing_mode  = "UTILIZATION"
      capacity_scaler = 1.0
    }
  }
...
resource "google_compute_backend_service" "plant_health" {
  count       = var.enable_plant ? 1 : 0
  name        = "${var.environment}-plant-health-backend"
  project     = var.project_id
  protocol    = "HTTPS"
  port_name   = "http"
  timeout_sec = 30
  ...
  load_balancing_scheme = "EXTERNAL_MANAGED"
  health_checks         = [google_compute_health_check.plant_health[0].id]

  dynamic "backend" {
    for_each = contains(keys(var.backend_negs), "plant_health") ? [1] : []
    content {
      group           = "projects/${var.project_id}/regions/${var.backend_negs["plant_health"].region}/networkEndpointGroups/${var.backend_negs["plant_health"].name}"
      balancing_mode  = "UTILIZATION"
      capacity_scaler = 1.0
    }
  }
...
resource "google_compute_url_map" "main" {
  ...
  dynamic "path_matcher" {
    ...
    content {
      ...
      # Route /health to CP health service
      path_rule {
        paths   = ["/health"]
        service = google_compute_backend_service.cp_health[0].id
      }
    }
  }
  ...
  dynamic "path_matcher" {
    ...
    content {
      ...
      # Route /health to PP health service
      path_rule {
        paths   = ["/health"]
        service = google_compute_backend_service.pp_health[0].id
      }
    }
  }
  ...
  dynamic "path_matcher" {
    ...
    content {
      ...
      # Route /health to Plant health service
      path_rule {
        paths   = ["/health"]
        service = google_compute_backend_service.plant_health[0].id
      }
    }
  }
}
