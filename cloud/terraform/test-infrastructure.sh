...
if curl -s -k "https://$CUSTOMER_DOMAIN/api/health" | grep -q "ok\|healthy\|status"; then
    ...
    BACKEND_URL=$(gcloud run services describe waooaw-api-demo --region=asia-south1 --format="get(status.url)")
...
BACKEND_URL=$(gcloud run services describe waooaw-api-demo --region=asia-south1 --format="get(status.url)" 2>/dev/null)
CUSTOMER_URL=$(gcloud run services describe waooaw-portal-demo --region=asia-south1 --format="get(status.url)" 2>/dev/null)
