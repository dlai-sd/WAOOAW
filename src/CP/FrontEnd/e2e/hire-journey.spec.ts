...
test.describe('CP hire journey', () => {
  test('moves from discovery to DMA runtime truth with approval-gated YouTube readiness', async ({ page }) => {
    ...
    await expect(page.getByText('YouTube channel setup')).toBeVisible()
    await page.getByTestId('cp-hire-setup-youtube-option-cred-youtube-001').click()
    await page.getByTestId('cp-hire-setup-next').click()

    await expect(page.getByText('Step 4 of 4')).toBeVisible()
    await page.getByTestId('cp-hire-setup-review-action-1').click()
    await expect(page.getByText('Step 1 of 4')).toBeVisible()
    await expect(page.getByTestId('cp-hire-setup-nickname')).toHaveValue('YouTube Growth Desk')
    await page.getByTestId('cp-hire-setup-next').click()
    await page.getByTestId('cp-hire-setup-next').click()
    await expect(page.getByText('Step 3 of 4')).toBeVisible()
    await page.getByTestId('cp-hire-setup-next').click()
    await expect(page.getByText('Step 4 of 4')).toBeVisible()
    await page.getByTestId('cp-hire-setup-review-action-3').click()
    await expect(page.getByText('Step 3 of 4')).toBeVisible()
    await page.getByTestId('cp-hire-setup-next').click()
    ...
    await expect(page.getByText('Digital Marketing Agent is now in runtime setup')).toBeVisible()
    ...
    await page.getByText('Continue to identity').click()
    await page.getByText('Continue to connection').click()
    await page.getByText('Continue to operating plan').click()
    await page.getByText('Open operating plan').click()
    ...
    await expect(page.getByText('Publish readiness', { exact: true })).toBeVisible()
    await expect(page.getByText('Clinic Growth Studio ready to attach')).toBeVisible()
