...
export function PerformancePanel(props: { instance: MyAgentInstanceSummary }) {
  ...
  useEffect(() => {
    ...
    getContentRecommendations(hiredInstanceId)
      ...
      .catch(() => {
        if (!cancelled) setRecsError('Failed to load content insights.')
      })
      .finally(() => {
    ...
    <div>
      ...
      <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid var(--colorNeutralStroke2)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
          ...
          {recsLoading ? <Spinner size="tiny" /> : null}
        </div>
        {recsError ? (
          <FeedbackMessage intent="error" title="Insights unavailable" message={recsError} />
        ) : recsLoading ? null : !recommendations || recommendations.total_posts_analyzed === 0 ? (
          <div style={{ opacity: 0.75, fontSize: '0.9rem' }}>
            Your agent is analyzing your content — first insights appear within 24–48 hours
          </div>
        ) : recommendations.total_posts_analyzed < 3 ? (
          <div style={{ opacity: 0.75, fontSize: '0.9rem' }}>
          ...
          <div style={{ display: 'grid', gap: '0.75rem' }}>
            <div>
              <div style={{ fontSize: '0.8rem', opacity: 0.7, marginBottom: '0.3rem' }}>Best posting hours</div>
              <div>{recommendations.best_posting_hours.map(formatPostingHour).join(', ') || 'No pattern yet'}</div>
            </div>
            <div>
              <div style={{ fontSize: '0.8rem', opacity: 0.7, marginBottom: '0.3rem' }}>Top content dimensions</div>
              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                {recommendations.top_dimensions.map((dimension) => (
                  <Badge key={dimension} appearance="outline">{dimension}</Badge>
                ))}
              </div>
            </div>
            <div>
              <div style={{ fontSize: '0.8rem', opacity: 0.7, marginBottom: '0.3rem' }}>Average engagement rate</div>
              <div>{(recommendations.avg_engagement_rate * 100).toFixed(1)}%</div>
            </div>
            <blockquote style={{ margin: 0, paddingLeft: '1rem', borderLeft: '3px solid #667eea', color: 'inherit' }}>
