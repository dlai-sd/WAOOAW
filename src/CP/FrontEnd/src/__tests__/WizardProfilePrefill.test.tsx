...
const { getProfileMock, updateProfileMock } = vi.hoisted(() => ({
  ...
  updateProfileMock: vi.fn(async (payload) => ({
    id: 'user-1',
    email: 'test@example.com',
    ...payload,
  })),
...
describe('Wizard profile prefill', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('prefills brand and location from profile when workspace is empty', async () => {
    await renderThemeStep()

    await waitFor(() => {
      expect(screen.getByLabelText('Brand name')).toHaveValue('TestCo')
    })
    expect(screen.getByLabelText('Location')).toHaveValue('Delhi')
  })

  it('keeps workspace values when they already exist', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('Location')).toHaveValue('Mumbai')
    })
  })
