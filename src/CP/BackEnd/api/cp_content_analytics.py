...
async def get_content_recommendations_proxy(
    hired_instance_id: str,
    request: Request,
    _: User = Depends(get_current_user),
    plant: PlantGatewayClient = Depends(get_plant_gateway_client),
):
    """Proxy content recommendations from Plant."""
    try:
    ...
    logger.info("CP proxied content recommendations for hired_instance_id=%s", hired_instance_id)
    return resp.json
