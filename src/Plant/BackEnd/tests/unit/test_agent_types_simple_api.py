...
def test_list_agent_types_returns_two_definitions(test_client):
    ...
    assert "marketing.healthcare.v1" in ids
    assert "trading.delta_futures.v1" in ids
...
def test_get_agent_type_by_id(test_client):
    ...
    assert res.status_code == 200
    data = res.json()
    assert data["agent_type_id"] == "marketing.healthcare.v1"
    assert data["version"]
...
def test_upsert_agent_type_updates_definition(test_client):
    existing = test_client.get("/api/v1/agent-types/marketing.healthcare.v1")
    assert existing.status_code == 200
    payload = existing.json()
    payload["version"] = "1.0.1"

    res = test_client.put("/api/v1/agent-types/marketing.healthcare.v1", json=payload)
    assert res.status_code == 200
    assert res.json()["version"] == "1.0.1"

    after = test_client.get("/api/v1/agent-types/marketing.healthcare.v1")
