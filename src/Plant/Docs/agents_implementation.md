...
|---|---|---|---|---|
| **PP-PLANT-1: Reference agents demo & publish flow** | **1.1 List reference agents**: show available reference agents in PP so ops can demo “Try Before Hire”. | PP FE: `src/PP/FrontEnd/src/pages/*` (new page, e.g. `ReferenceAgents.tsx`), PP FE routes: `src/PP/FrontEnd/src/App.tsx`, PP FE client: `src/PP/FrontEnd/src/services/gatewayApiClient.ts` | Plant: `GET /api/v1/reference-agents` | PP page lists agents with name/type/spec, handles loading/errors, and is reachable via router + nav. |
|  | **1.2 Run reference agent (draft)**: run an agent with `customer_id`, `plan_id`, and show draft output. | PP FE: new page; PP BE (optional proxy): `src/PP/BackEnd/api/reference_agents.py` or use existing `/api/*` proxy in `src/PP/BackEnd/main_proxy.py` | Plant: `POST /api/v1/reference-agents/{agent_id}/run` | PP can run a reference agent, display returned `draft`, and surface correlation_id on errors. |
|  | **1.3 Approval-assisted publish**: support `approval_id` entry and `do_publish=true` to simulate the publish gate. | PP FE: `src/PP/FrontEnd/src/pages/GovernorConsole.tsx` (wire to Plant publish approval UX), PP FE client updates | Plant: publish gating via hooks in reference agent run endpoint | Attempt without `approval_id` shows deny reason; attempt with `approval_id` shows `published=true` and records a `publish_action` usage event. |
| **PP-PLANT-2: Usage & budget visibility in PP** | **2.1 Usage events list**: display usage events by `customer_id` and optional `agent_id` so PP can explain billing/budgets. | PP FE: `src/PP/FrontEnd/src/pages/CustomerManagement.tsx` (replace static table with live data) or a new Usage page; PP FE client | Plant: `GET /api/v1/usage-events` | PP shows a table of events (type/model/tokens/cost/correlation_id) and can filter by customer. |
|  | **2.2 Usage aggregation**: chart/bucket usage per UTC day/month for budget reporting. | PP FE: new “Usage Dashboard” page; PP FE client | Plant: `GET /api/v1/usage-events/aggregate` | PP renders deterministic buckets and matches Plant’s UTC window semantics (day/month). |
|  | **2.3 Budget explainability**: when a call is denied due to budgets, show plan/month window reset time from error details. | PP FE: error UI (`src/PP/FrontEnd/src/components/ApiErrorPanel.tsx`) and relevant pages | Plant: `UsageLimitError` reasons `monthly_budget_exceeded`, `metering_required_for_budget`, `metering_envelope_*` | PP error panel renders `reason`, `details.window_resets_at`, and `correlation_id` consistently. |
| **PP-PLANT-3: Policy denial audit visibility** | **3.1 List policy denials**: expose the denial audit log so ops can debug “why denied”. | PP FE: new page (e.g. `PolicyDenials.tsx`); PP FE routes; PP FE client | Plant: `GET /api/v1/audit/policy-denials` | PP lists denial records with correlation_id/decision_id/action/reason/path and supports `limit`. |
|  | **3.2 Drilldown**: click a denial → show details JSON + recommended operator next action. | PP FE: new detail view or inline panel | Plant: denial record `details` payload | PP supports a simple drilldown that surfaces full `details` and correlation_id. |
| **PP-PLANT-4: AgentSpec tooling in PP (spec validation & schema)** | **4.1 Fetch AgentSpec JSON schema**: allow PP to download/display schema for external validation. | PP FE: new page (e.g. `AgentSpecTools.tsx`); PP FE client | Plant: `GET /api/v1/agent-mold/schema/agent-spec` | PP renders schema JSON and supports download/copy; schema fetch is cached client-side. |
