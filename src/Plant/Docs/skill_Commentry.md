...
- Scope: implement a Gateway “API Request Pipeline” that mints/propagates correlation IDs, injects tenant context, validates requests against the upstream Plant Backend OpenAPI schema, and enforces a simple circuit breaker.

### US-492-2 implementation notes

- Plant Gateway: added `RequestPipelineMiddleware` with:
	- Correlation ID minting (`X-Correlation-ID`) and `request.state.correlation_id`.
	- Tenant injection (`request.state.tenant_id`) derived from `request.state.customer_id` / JWT claims.
	- OpenAPI request validation backed by upstream `/openapi.json` with in-memory TTL caching.
	- Circuit breaker: opens after 3 failures within 10 seconds (returns 503 without calling downstream).
- Plant Gateway: proxy handler now forwards `X-Correlation-ID` and `X-Tenant-ID` upstream.
- Gateway tests: default `GW_OPENAPI_VALIDATE=false` for unit tests, with dedicated request-pipeline tests covering validation behavior.

### US-492-2 validation (Docker-only)

- Plant Gateway (Docker):
	- `docker compose -f docker-compose.local.yml up -d --build plant-gateway`
	- `docker compose -f docker-compose.local.yml exec -T plant-gateway pytest -q middleware/tests/test_request_pipeline.py middleware/tests/test_proxy.py` → 14/14 passed.

### US-492-3 kickoff

- Scope: implement graceful client-side retries with exponential backoff and user-friendly error messages (no stack traces), while preserving RFC7807 Problem Details for debugging.

### US-492-3 implementation notes

- CP + PP FrontEnd: `gatewayRequestJson()` now performs automatic retries for transient failures:
	- Retryable statuses: 429, 500, 502, 503, 504
	- Retryable fetch errors: network/TypeError + timeout/AbortError
	- Backoff delays: 1s, 2s, 4s (3 retries; 4 total attempts)
- Sanitizes user-facing error messages to avoid leaking stack traces (replaces with a generic message).
- Never retries 401: fail-closed and trigger existing auth-expired flows.

### US-492-3 validation (Docker-only)
