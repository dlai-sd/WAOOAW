...
@lru_cache(maxsize=1)
def _fernet() -> Fernet:
    secret = (os.getenv("PP_EXCHANGE_CREDENTIALS_SECRET") or os.getenv("JWT_SECRET") or "").strip()
    if not secret:
        raise RuntimeError("PP_EXCHANGE_CREDENTIALS_SECRET or JWT_SECRET must be set")
