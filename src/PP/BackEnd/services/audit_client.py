"""PP AuditClient — PP-N4 — write audit events to Plant Audit API.

Design mirrors CP's audit_client.py:
- Wraps POST /api/v1/audit/events on Plant backend.
- All calls are fire-and-forget via asyncio.create_task() — audit never
  blocks the business flow.
- If Plant Audit API is unavailable or slow, the exception is silently
  caught, logged as a warning, and swallowed.
...
Usage::
    ...
    client = AuditClient(base_url=settings.plant_base_url, key=settings.AUDIT_SERVICE_KEY)

    fire_and_forget_audit(client.log(
        screen="pp_approvals",
        action="approval_minted",
        outcome="success",
        detail=f"Approval {approval_id} minted for customer {customer_id}",
    ))
"""
...
class AuditClient:
    ...
    async def log(
        ...
        *,
        ...
        # If no key is configured, skip sending to avoid unauthenticated calls.
        if not self._key:
            logger.debug(
                "audit_client: AUDIT_SERVICE_KEY not set — skipping screen=%s action=%s",
                screen,
                action,
            )
            return
...
def fire_and_forget_audit(coro: Any) -> None:  # type: ignore[type-arg]
    ...
    The audit call is completely non-blocking. Any exception inside
    AuditClient.log() is swallowed.
