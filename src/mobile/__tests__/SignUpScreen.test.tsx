...
const fillRequiredFields = (
  ...
  fireEvent.press(getByLabelText('I agree to the Terms of Service and Privacy Policy')); // consent
};
...
describe('SignUpScreen', () => {
  ...
  it('calls onSignInPress when sign in link is pressed', () => {
    const { getByText } = render(<SignUpScreen onSignInPress={mockOnSignInPress} />);
  ...
  it('validates website URL format when provided', async () => {
    ...
    fireEvent.press(getByText('Sign Up'));
    expect(await findByText('Enter a valid URL (e.g. https://example.com)')).toBeTruthy();
  });
  ...
  it('accepts valid https:// website URL', async () => {
    ...
    fireEvent.press(getByText('Sign Up'));
    await waitFor(() => {
      expect(queryByText('Enter a valid URL (e.g. https://example.com)')).toBeNull();
    });
  });
  ...
  it('validates GST number format when provided', async () => {
    ...
    fireEvent.press(getByText('Sign Up'));
    expect(await findByText('Enter a valid GSTIN (e.g. 22AAAAA0000A1Z5)')).toBeTruthy();
  });
  ...
  it('accepts valid GSTIN format', async () => {
    ...
    fireEvent.press(getByText('Sign Up'));
    await waitFor(() => {
      expect(queryByText('Enter a valid GSTIN (e.g. 22AAAAA0000A1Z5)')).toBeNull();
    });
  });

  it('clears field error when user types', async () => {
    const { getByLabelText, getByText, queryByText, findByText } = render(<SignUpScreen />);
    fireEvent.press(getByText('Sign Up'));
  ...
  it('registers with correct payload including phoneCountry and phoneNationalNumber', async () => {
    (RegistrationService.registerAndStartOTP as jest.Mock).mockResolvedValue(OTP_SUCCESS);
    ...
    await waitFor(() => {
      ...
      expect(mockOnRegistrationSuccess).toHaveBeenCalledWith(
        'REG-123', 'OTP-123', 'email', 't***t@example.com'
      );
    });
  });
  ...
  it('handles email already registered error', async () => {
    ...
    expect(await findByText('This email is already registered. Please sign in.')).toBeTruthy();
  });
  ...
  it('handles phone already registered error', async () => {
    ...
    expect(await findByText('This phone is already registered. Please sign in.')).toBeTruthy();
  });

  it('handles generic registration error', async () => {
    (RegistrationService.registerAndStartOTP as jest.Mock).mockRejectedValue(new Error('Network error'));
