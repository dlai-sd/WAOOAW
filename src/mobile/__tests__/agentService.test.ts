...
describe("AgentService", () => {
  ...
  describe("getAgent", () => {
    it("should fetch single agent by ID via catalog endpoint", async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith(
        "/api/v1/catalog/agents/agent-123",
      );
  ...
  describe("searchAgents", () => {
    ...
    const mockCatalog = [
      ...
      },
      {
        release_id: "rel-2",
        id: "a2",
        public_name: "Sales Agent",
        short_description: "Sales",
        industry_name: "sales",
        job_role_label: "SDR",
    ...
    it("should combine query and industry filter", async () => {
      ...
      const result = await agentService.searchAgents("agent", {
        industry: "sales",
      });
  ...
  describe("listSkills", () => {
    ...
    it("should fetch skills with category filter", async () => {
      ...
      await agentService.listSkills({
        category: "domain_expertise",
        limit: 20,
      });

      expect(apiClient.get).toHaveBeenCalledWith(
        "/api/v1/skills?category=domain_expertise&limit=20",
      );
    });
  });
  ...
  describe("buildQueryString", () => {
    ...
    it("should build query string for skills endpoint", async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith(
        "/api/v1/skills?category=technical&limit=5",
      );
    });
  });
});
