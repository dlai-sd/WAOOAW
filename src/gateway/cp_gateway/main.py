"""
Customer Portal (CP) Gateway - Main Application

Serves customer-facing requests, routes to Plant service.
Enforces trial mode, budget limits, audit logging.

Middleware Chain:
1. ErrorHandlingMiddleware: Catch all exceptions
2. AuditLoggingMiddleware: Log all requests
3. AuthMiddleware: JWT validation
4. PolicyMiddleware: Trial/Governor/Sandbox
5. BudgetGuardMiddleware: Budget enforcement

No RBAC (customer portal doesn't need role-based permissions).
"""
...
# Create FastAPI app
app = FastAPI(
    title="WAOOAW Customer Portal Gateway",
    description="Constitutional enforcement gateway for customer-facing requests",
    version="1.0.0",
...
if ENVIRONMENT == "development":
    ...
    # Production CORS (restrict to waooaw.com domains)
    app.add_middleware(
        CORSMiddleware,
        allow_origins=[
            "https://waooaw.com",
...
async def health():
    ...
    return {"status": "healthy", "gateway": "CP", "version": "1.0.0"}
...
async def proxy_to_plant(path: str, request: Request):
    ...
    - Auth: JWT validated, user_id/customer_id attached to request.state
...
async def startup_event():
    logger.info("=" * 80)
    logger.info("🚀 CP Gateway Starting")
    logger.info("=" * 80)
...
async def shutdown_event():
    logger.info("CP Gateway shutting down...")
    await plant_client.aclose()
    if feature_flag_service:
        feature_flag_service.close()
    logger.info("CP Gateway shutdown complete")
