"""
Error Handling Middleware - GW-105

RFC 7807 Problem Details wrapper for all gateway errors.
Converts exceptions to standardized error responses.

Catches all exceptions from middleware chain, formats as RFC 7807 Problem Details.
"""
...
class ErrorHandlingMiddleware(BaseHTTPMiddleware):
    """
    Error Handling Middleware for CP and PP Gateways.
    
    Wraps all middleware in try/except, converts exceptions to RFC 7807 format.
    ...
    - permission-denied: RBAC denied
    - trial-limit-exceeded: Trial 10 tasks/day limit
    - trial-expired: Trial period expired
    - budget-exceeded: Budget limit reached
    - approval-required: Governor approval needed
    - service-unavailable: Downstream service down
    - internal-server-error: Unexpected error
    
    Configuration:
    - ENVIRONMENT: "production" or "development" (show stack traces in dev)
    """
    ...
    def __init__(self, app, environment: str = "production", include_trace: bool = False):
        super().__init__(app)
        self.environment = environment.lower()
        self.include_trace = include_trace or (environment.lower() == "development")
    
    async def dispatch(self, request: Request, call_next):
        """
        Wrap middleware chain, catch exceptions, format as RFC 7807.
        """
        try:
            ...
            # FastAPI HTTPException (already has status_code and detail)
            ...
            # Unexpected error
            return self._format_unexpected_error(e, request)
    ...
    def _format_http_exception(
        ...
        error_type = self._infer_error_type(status_code, detail)
        ...
        # Add extra fields from exception (if dict)
    ...
    def _format_unexpected_error(
        ...
        problem_details = {
            ...
            "status": status_code,
            "detail": str(exc) if self.include_trace else "An unexpected error occurred",
            "instance": str(request.url.path)
        }
    ...
    def _infer_error_type(self, status_code: int, detail: str) -> str:
        """
        ...
        if "unauthorized" in detail_lower or "auth" in detail_lower:
            return "unauthorized"
        elif "permission" in detail_lower or "forbidden" in detail_lower:
            return "permission-denied"
        elif "trial" in detail_lower and "limit" in detail_lower:
            return "trial-limit-exceeded"
        elif "trial" in detail_lower and "expired" in detail_lower:
            return "trial-expired"
        elif "budget" in detail_lower:
        ...
        elif "conflict" in detail_lower:
            return "conflict"
        elif "timeout" in detail_lower:
            return "gateway-timeout"
        elif "unavailable" in detail_lower or "service" in detail_lower:
