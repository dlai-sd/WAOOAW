...
Version: 1.0
Purpose: Test JWT validation, claim extraction, error handling

Test Coverage:
- Valid JWT with all required claims
- Expired JWT (401 with token-expired error)
- Invalid signature (401 with invalid-token error)
- Missing Authorization header (401 with unauthorized error)
- Invalid Bearer format (401 with invalid-token-format error)
- Missing required claims (401 with invalid-token error)
- Trial mode validation (trial_expires_at required)
- Public endpoints bypass (health, docs)
- Governor detection (admin + governor_agent_id)
- Request state enrichment (jwt, user_id, roles, trial_mode)
"""
...
def test_middleware_performance():
    ...
    print(f"✓ Average middleware overhead: {avg_time_ms:.2f}ms")
