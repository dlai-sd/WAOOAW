...
from waooaw.agents.wowintegration import WowIntegration
...
def test_agent_initialization(agent):
    ...
    assert agent.coe_name == "WowIntegration"
    assert agent.display_name == "WowIntegration"
    assert agent.tier == 6
...
def test_agent_capabilities(agent):
    ...
    expected_capabilities = {"adapters": ["rest_adapter", "graphql_adapter", "soap_adapter", "grpc_adapter"], "api_management": ["versioning", "rate_limiting", "quota_management", "api_keys"], "transformation": ["data_mapping", "format_conversion", "schema_translation", "payload_validation"], "webhooks": ["webhook_management", "event_subscriptions", "payload_transformation", "retry_policies"]}
...
def test_agent_dependencies(agent):
    ...
    expected_deps = ["WowVisionPrime", "WowAgentFactory", "WowCommunication"]
